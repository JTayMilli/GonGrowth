...
for (i in 1:bootnum) {
  ...
  second_stage <- lm(lnGDPPCp5 ~ lnGDPPC + Gini + Gini2 + lnGG + lnGG2 + n  + pOld + factor(PR) + factor(Country) + factor(Year), data = data)#+ factor(PR)
...
#bootcoef
fstbootcoefM <- matrix(unlist(bootcoef), nrow = length(bootcoef), byrow = TRUE)
...
for (i in 1:bootnum) {
  ...
  # Step 1: First stage regression
  first_stage <- lm(Gini ~ lnGDPPC + OPS + TWWI + n + pOld + factor(Country) + factor(Year) , data = data)
  #summary(first_stage)#+ factor(PR)
  fstbootcoef[i] <- list(summary(second_stage)$coefficients[1:6])
  ...
  # Step 2: Second stage regression
  second_stage <- lm(d5pGDPPC ~ lnGDPPC + Gini + Gini2 + lnGG + lnGG2 + n  + pOld + factor(Country) + factor(Year), data = data)#+ factor(PR)
...
for (i in 1:bootnum) {
  ...
  # Step 1: First stage regression
  first_stage <- lm(Gini ~ OPS + TWWI + n + FD + pOld + factor(Country) + factor(Year) , data = data)
  #summary(first_stage)#+ factor(PR)
  fstbootcoef[i] <- list(summary(second_stage)$coefficients[1:6])
  ...
  # Step 2: Second stage regression
  second_stage <- lm(d5pGDPPC ~ Gini + Gini2 + FD + FDG + FDG2 + n + pOld + factor(Country) + factor(Year), data = data)#+ factor(PR)
