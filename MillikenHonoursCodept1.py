...
CountryList = ['Albania', 'Algeria', 'Argentina', 'Armenia', 'Australia',
       ...
       'Montenegro', 'Morocco', 'Mozambique', 'Nepal', 'Netherlands',
       'New Zealand', 'Nicaragua', 'Nigeria', 'Norway', 'Oman',
       'Pakistan', 'Panama', 'Paraguay', 'Peru', 'Philippines', 'Poland',
       'Portugal', 'Qatar', 'Romania', 'Rwanda', 'Saudi Arabia', 'Serbia',
       'Singapore', 'Tajikistan', 'Thailand', 'Togo',
       'Trinidad and Tobago', 'Tunisia', 'Turkmenistan', 'Uganda',
       'Ukraine', 'Uruguay', 'Zambia', 'Zimbabwe'] #China, United States, Russia
...
TWWI['TWWIm5'] = TWWI.groupby('Country')['TWWI'].shift(5)
...
tfp_gini = tfp_gini.sort_values(by = ["Country", "Year"])
...
GDPpc_Data = GDPpc_Data.replace("not found",np.nan).replace(0,np.nan).dropna()

GDPpc_Data = GDPpc_Data[GDPpc_Data.Country.isin(CountryList)]

GDPpc_Data["maxGDPPC"] = GDPpc_Data.groupby(['Year'])['GDPPC'].transform('max')
GDPpc_Data['RelGDPPC']  = GDPpc_Data['GDPPC'] / GDPpc_Data["maxGDPPC"]
...
GNId = GNId.replace("not found",np.nan).replace(0,np.nan).dropna()
GNId.Year = GNId.Year.astype(int)

GDPpc_Data = GDPpc_Data.merge(GNId[['Country', 'Year', 'GNI']], on = ['Country', 'Year'], how = 'left')

TGall = tfp_gini.merge(GDPpc_Data, on = ['Country', 'Year'], how = 'outer')
TGall = TGall[TGall.GDPPC != 0]

######## GNI regression only
#TGall.GDPPC = TGall.GNI / TGall.pop
#####
...
for x in TGall["Country"].unique():
    ...
    temp["dpGDPPC"] = temp['GDPPC'].diff(periods=1) / temp['GDPPC']
    temp["d5pGDPPC"] = temp['GDPPC'].diff(periods=5) / temp['GDPPC']
...
# %%
#TGall.groupby(['Year'])['lnGDPPC'].transform('max')
...
TGall["lnGG2"] = TGall["lnGDPPC"].mul(TGall['Gini']).mul(TGall['Gini'])
TGall["lnGGm5"] = TGall.groupby('Country')["lnGG"].shift(5)
TGall["lnGG2m5"] = TGall.groupby('Country')["lnGG2"].shift(5)
TGall["lnGDPPCm5"] = TGall.groupby('Country')["lnGDPPC"].shift(5)
TGall["lnGDPPCp5"] = TGall.groupby('Country')["lnGDPPC"].shift(-5)
...
# %%
TGall[["Country", "Year", "lnGDPPCm5", "lnGDPPC", "lnGDPPCp5", "Gini", "Ginim5"]].dropna().head(30)
...
# %%
TGall['OPS'] = TGall['OilW'] * TGall['dlnP']
OPSavg = ( TGall[["Year", 'Country', 'OPS']].groupby('Country')[["Year",'OPS']]
        .rolling(5, min_periods=1, on='Year', closed = "right")
        .mean().reset_index().drop(columns = "level_1")
        ).rename(columns = {'OPS':'OPSavg'})  ##calculating oil averge over previous 5 years
#OPS['OPSm5'] = OPS.groupby('Country')['OPS'].shift(5)
TGall = TGall.merge(OPSavg, on= ['Year','Country'], how = "left")
TGall = TGall[TGall.Country != 'not found']
...
# %%
FinDev = pd.read_excel("FinDev.xlsx").replace(0, np.nan).dropna()
FinDev.Year = FinDev.Year.astype(int)
FinDev = FinDev[FinDev['Country'] != "Aruba, Kingdom of the Netherlands"]
FinDev = FinDev[FinDev['Country'] != "Curaçao, Kingdom of the Netherlands"] #countries that don't matter
FinDev = FinDev[FinDev['Country'] != "Sint Maarten, Kingdom of the Netherlands"]
FinDev = FinDev.rename(columns = {'FDI': 'FD'})
FinDev = FinDev.merge(( FinDev.groupby('Country')[["Year","FD"]]
                   .rolling(5, min_periods=1, on='Year', closed = "right")
                   .mean().reset_index().drop(columns = "level_1")
                   .rename(columns = {'FD':'FDavg'}) ), on = ["Country", "Year"], how = "left")#.fillna(0) 

FinDev["Country"] = cc.convert(names = FinDev['Country'], to = "name_short")
FinDev = FinDev.replace("not found",np.nan).replace(0,np.nan).dropna()

TGall = TGall.merge(FinDev, on=['Year','Country'], how = 'left')
...
# %%
OldData = ( pd.read_csv("API_SP.POP.65UP.TO.ZS_DS2_en_csv_v2_48820.csv", skiprows=3)
         .drop(["Country Code", "Indicator Name", "Indicator Code", "Unnamed: 68"], axis = 1)
         .rename(columns={"Country Name": "Country"})
)
OldData = pd.melt(OldData, id_vars=["Country"], var_name="Year", value_name="pOld")
OldData.Year = OldData.Year.astype(int)

# %%
OldData["Country"] = cc.convert(names = OldData['Country'], to = "name_short")
OldData = OldData.replace("not found",np.nan).dropna()
TGall = TGall.merge(OldData, on=["Country", "Year"], how="left")

# %%
UrData = ( pd.read_csv("API_SP.URB.TOTL.IN.ZS_DS2_en_csv_v2_43721.csv", skiprows=3)
         .drop(["Country Code", "Indicator Name", "Indicator Code", "Unnamed: 68"], axis = 1)
         .rename(columns={"Country Name": "Country"})
)
UrData = pd.melt(UrData, id_vars=["Country"], var_name="Year", value_name="Ur")
UrData.Year = UrData.Year.astype(int)

# %%
UrData["Country"] = cc.convert(names = UrData['Country'], to = "name_short")
UrData = UrData.replace("not found",np.nan).dropna()
TGall = TGall.merge(UrData, on=["Country", "Year"], how="left")

# %%
import re

DemData = ( pd.read_excel("Country_and_Territory_Ratings_and_Statuses_FIW_1973-2024.xlsx", sheet_name = "Country Ratings, Statuses ", skiprows=1)
           .rename(columns = {'Year(s) Under Review': 'Country'}) )
DemData["Country"] = cc.convert(names = DemData['Country'], to = "name_short")
yrs = DemData.iloc[:, 1::3].columns
DemData = pd.concat([DemData['Country'], DemData.iloc[:, 1::3]], axis = 1).drop(0)

DemData = DemData.melt(id_vars=["Country"], var_name="Year", value_name="PR").replace("-", np.nan)
TGall = TGall.merge(DemData, on = ["Country", "Year"], how  = "left")
...
# %%
TGall["nm5"] = TGall.groupby('Country')["n"].shift(5)
TGall["OPSm5"] = TGall.groupby('Country')["OPS"].shift(5)
TGall["TWWIm5"] = TGall.groupby('Country')["TWWI"].shift(5)
TGall["FDm5"] = TGall.groupby('Country')["FD"].shift(5)
TGall["PRm5"] = TGall.groupby('Country')["PR"].shift(5)
TGall["pOldm5"] = TGall.groupby('Country')["pOld"].shift(5)
TGall["Urm5"] = TGall.groupby('Country')["Ur"].shift(5)

# %%
TGall["GFDm5"] = TGall["Ginim5"] * TGall["FDm5"]
TGall["G2FDm5"] = TGall["Gini2m5"] * TGall["FDm5"]
...
TGall['Gini'] = TGall['Gini'].replace(0, np.nan)
X = TGall[["Year", "Country", "dpTWWI", "TWWI", "Gini", "OPS", "lnGDPPC"]].replace(0, np.nan).dropna(axis=0)
...
X = X[["dpTWWI", "TWWI", "Gini", "OPS", "lnGDPPC"]]
X = X[(np.abs(stats.zscore(X)) < 3).all(axis=1)]
X['B0'] = 1
# model = linear_model.LinearRegression(fit_intercept=False)
#reg = model.fit(X[["dpTWWI", 'TWWI', 'OilWP', 'lnGDPPC', 'B0']], X['Gini'])
instr = "OPS"
alpha = np.cov(X[instr], X.Gini)[0][1] / np.cov(X[instr], X.lnGDPPC)[0][1]
instr = "TWWI"
b = np.cov(X[instr], X.Gini)[0][1] / np.cov(X[instr], X.lnGDPPC)[0][1]
print(alpha)
print(b)

## the beta on gdppc is 100 times higher with the other variables included
AdjGDP = alpha #7.789e-01 #model.coef_[3]#####THIS LINE MATTERS, choosing to import from R rn instead of Python regressio
...
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 5 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
#X = X[X.Country.isin(CountryList)] #make sure that the country is in our list of countries
X.to_csv("tansferdata.csv")
X.describe()
...
# %%
TGall['GFDavg'] = TGall['FDavg'] * TGall['Ginim5']
TGall['G2FDavg'] = TGall['FDavg'] * TGall['Gini2m5']
TGall['IVGFDavg'] = TGall['FDavg'] * TGall['IVGinim5']
TGall['IVG2FDavg'] = TGall['FDavg'] * TGall['IVGini2m5']

# %%
X = TGall[["Year", "Country", "Gini", "lnGDPPC", "lnGDPPCp5", "n", "OPS", "TWWI", "PR"]].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country", "PR"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
X = X[X.Country.isin(CountryList)] #make sure that the country is in our list of countries
X.to_csv("tansferdata.csv")
...
# %%
X = TGall[["Year", "Country", "Ginim5", "lnGDPPCm5", "lnGDPPC", "nm5", "pOldm5", "Urm5", "TWWIm5", "OPSm5"]].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
...
# %%
X = TGall[["Year", "Country", "Ginim5", "lnGDPPC", "lnGDPPCm5", "RelGDPPCm5", "nm5", "pOldm5", "Urm5", "TWWIavg", "dpTWWIavg", "OPSavg"]].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
X = X[X.Country.isin(CountryList)] #make sure that the country is in our list of countries
X.to_csv("tansferdata3.csv")
...
# %%
X = TGall[["Year", "Country", "Ginim5", "lnGDPPC", "lnGDPPCm5", "FDm5", "pOldm5", "nm5", "Urm5", "TWWIavg", "dpTWWIavg", "OPSavg", "PRm5"]].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country", "PRm5"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
X = X[X.Country.isin(CountryList)] #make sure that the country is in our list of countries
X.to_csv("tansferdata2.csv")

# %%
np.corrcoef(X.FDm5,X.lnGDPPCm5)

# %%
X = TGall[["Year", "Country", "Ginim5", "lnGDPPCm5", "lnGDPPC", "n"]].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
...
# %%
X = TGall[["Year", "Country", "Gini", "lnGDPPC", "lnGDPPCp5", "n"]].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
...
# %%
X = TGall[["Year", "Country", "Gini", "lnGDPPC", "lnGDPPCp5", "n", "OPS", "TWWI"]].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
...
# %%
X = TGall[["Year", "Country", "lnGGm5", "Ginim5", "lnGDPPC", "lnGDPPCm5", 'Gini2m5', 'lnGG2m5', "nm5", "pOldm5", "Urm5", "OPSavg", "TWWIavg", "dpTWWIavg", "PRm5"]].replace(0,np.nan).dropna()
...
X = TGall[["Year", "Country", "lnGGm5", "Ginim5", "lnGDPPC", "lnGDPPCm5", 'Gini2m5', 'lnGG2m5', "nm5", "pOldm5", "Urm5", "OPSavg", "TWWIavg", "dpTWWIavg"]].replace(0,np.nan).dropna()
...
Arti['Ginim5'] = range(1,100) ###This goes first!
Arti['Country'] = 'France'
Arti['Year'] = 2018
Arti['lnGDPPCm5'] = 8
...
Arti['nm5'] = X.nm5.mean()
Arti['OPSavg'] = X.OPSavg.mean()
Arti['Urm5'] = X.Urm5.mean()
Arti['pOldm5'] = X.pOldm5.mean()
#Arti['abs_red'] = X.abs_red.mean()
Arti['TWWIavg'] = X.TWWIavg.mean()
Arti['dpTWWIavg'] = X.dpTWWIavg.mean()
Arti['Gini2m5'] = Arti['Ginim5'] ** 2
#Arti['IVGini3m5'] = Arti['IVGinim5'] ** 3
Arti['lnGGm5'] = Arti['Ginim5'] * Arti['lnGDPPCm5']
Arti['lnGG2m5'] = Arti['Gini2m5'] * Arti['lnGDPPCm5']
...
Arti['pred'] = Arti['pred'] - Arti['pred'][int(X.Ginim5.mean())] # make mean = 0
"""
...
"""
ax = sns.lineplot(data=Arti[['Ginim5', 'pred']], x = 'Ginim5', y = 'pred', color = "Black")
ax.set_xticks(range(0,101, 2))
#ax.set_yticks(range(-20, 111, 10))
ax.axvline(x = X.Ginim5.min(),    # Line on x = 2
        ymin = 100, # Bottom of the plot
        ymax = -100,
        color = 'red') # Top of the plot
ax.axvline(x = X.Ginim5.max(),    # Line on x = 2
        ymin = 100, # Bottom of the plot
        ymax = -100,
        color = 'red') # Top of the plot
ax.axvline(x = X.Ginim5.mean(),    # Line on x = 2
        ymin = 100, # Bottom of the plot
        ymax = -100,
        color = 'purple') # Top of the plot
ax.axvline(x = X.Ginim5.mean() - X.Ginim5.std(),    # Line on x = 2
        ymin = 100, # Bottom of the plot
        ymax = -100,
...
ax.set_ylim(Arti['pred'][int(X.Ginim5.mean())]-15, Arti['pred'][int(X.Ginim5.mean())]+15)
ax.set_xlim(X.Ginim5.min(), X.Ginim5.max())
...
X = TGall[["Year", "Country", "lnGGm5", "Ginim5", "lnGDPPC", "lnGDPPCm5", 'Gini2m5', 'lnGG2m5', "nm5", "pOldm5", "Urm5", "OPSavg", "TWWIavg", "dpTWWIavg", 'FDm5', 'GFDm5', 'G2FDm5']].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
...
Arti['Country'] = con
Arti['Year'] = 2018
conD = TGall[TGall.Country == con]
conG = float(conD[conD.Year == 2018]["Ginim5"])
lnG = float(conD[conD.Year == 2018]["lnGDPPCm5"])
Arti['lnGDPPCm5'] = lnG
FD = float(conD[conD.Year == 2018]["FDm5"])
Arti['FDm5'] = FD
#Arti['GDPPCm5'] = np.exp(Arti['lnGDPPCm5'][0])
#Arti['lnGDPPC2m5'] = Arti['lnGDPPCm5'] ** 2
Arti['nm5'] = X.nm5.mean()
...
Arti['GFDm5']  = Arti['FDm5'] * Arti['Ginim5']
Arti['G2FDm5']  = Arti['FDm5'] * Arti['Gini2m5']
...
ax.set_xlabel("Gini Coefficient")
ax.set_ylabel("% Change GDPPC (5 years)")
ax.set_title(f"{con}: lnGDPpc = {lnG:.1f}, FD = {FD:.2f}, Gini(2013) = {conG} \n\npurple: mean(Gini), brown: +- sd(Gini)")
plt.grid()
plt.show()
...
con = "Canada"
conD = TGall[TGall.Country == con]
lnG = conD[conD.Year == 2018]["lnGDPPCm5"]
print(float(lnG))
...
# %%
ed = X[X.lnGDPPC < 100]
ed = ed[ed.lnGDPPCm5 > 10]
ed = ed[ed.Year == 2018]
ed['dpGDPPC'] = ed['lnGDPPC'] - ed['lnGDPPCm5']
ed[['Country', 'lnGDPPCm5', 'Ginim5', 'dpGDPPC']]
sns.histplot(ed.Ginim5, binwidth= 1)
#sns.scatterplot(data = ed, x = 'IVGinim5', y = 'dpGDPPC')
...
# %%
def FinDevReg(lnGDPPCm5, FDavg):
    #X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', 'IVGini3m5', 'IVlnGG3m5', "IVlnG2Gm5", "IVlnG2G2m5", "IVlnG2G3m5", 'lnGDPPC2m5', "n", "OPS", "TWWIavg"]].replace(0,np.nan).dropna()
    #X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "d5pGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', 'IVGini3m5', 'IVlnGG3m5', "n", "OPS", "TWWIavg", "dpTWWIavg"]].replace(0,np.nan).dropna()
    X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'FDavg', 'GFDavg', 'G2FDavg']].replace(0,np.nan).dropna()
    #X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
    X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country"]))) < 3).all(axis=1)]
    X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
    X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
    ...
    Arti['GFDavg']  = Arti['FDavg'] * Arti['IVGinim5']
    Arti['G2FDavg']  = Arti['FDavg'] * Arti['IVGini2m5']
    #Arti['IVlnGG3m5'] = Arti['IVGini3m5'] * Arti['lnGDPPCm5']
    """
    Arti['RelGDPPCm5'] = Arti['GDPPCm5'][0] / TGall[TGall.Year == Arti.Year[0]].GDPPCm5.max()
    Arti['IVRelGGm5'] = Arti['RelGDPPCm5'] * Arti['IVGinim5']
...
size = 1
lnGDPPCm5 = 7
Arti, X = FinDevReg(lnGDPPCm5 , .1)
predList = [np.array(Arti['pred'])]
for i in range(1+size,10+size,size):
    Arti, X = FinDevReg(lnGDPPCm5, i * .1)
    predList.append(np.array(Arti['pred']))
...
hf2 = plt.figure()
ha2 = hf2.add_subplot(projection='3d')

X, Y = np.meshgrid(x, y)  # `plot_surface` expects `x` and `y` data to be 2D
ha2.plot_surface(X, Y, data)

ha2.set_xlabel('Gini')
ha2.set_ylabel('Financial Development')
ha2.set_zlabel('% Change GDPpc (5 years)')
#ha2.set_yticklabels(range(1,10+1))
ha2.set_title('% Change GDPpc (5 years)')
...
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', 'IVGini3m5', 'IVlnGG3m5', "IVlnG2Gm5", "IVlnG2G2m5", "IVlnG2G3m5", 'lnGDPPC2m5', "n", "OPS", "TWWIavg"]].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "d5pGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', 'IVGini3m5', 'IVlnGG3m5', "n", "OPS", "TWWIavg", "dpTWWIavg"]].replace(0,np.nan).dropna()
X = TGall[["Year", "Country", "IVGinim5", 'IVGini2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'FDavg', 'IVGFDavg', 'IVG2FDavg', 'd5pGDPPC']].replace(0,np.nan).dropna()
#X = TGall[["Year", "Country", "IVlnGGm5","IVGinim5", "lnGDPPC", "lnGDPPCm5", 'IVGini2m5', 'IVlnGG2m5', "n", "OPS", "TWWIavg", "dpTWWIavg", 'RelGDPPCm5', 'IVRelGGm5', 'IVRelGG2m5']].replace(0,np.nan).dropna()
X = X[(np.abs(stats.zscore(X.drop(columns = ["Year", "Country"]))) < 3).all(axis=1)]
X = X.groupby('Country').filter(lambda x: len(x) >= 5) #remove countries with less then 4 data points
X = X.groupby('Year').filter(lambda x: len(x) >= 10) #remove years with less then 10 entries
...
X2.columns = X2.columns.astype(str)
MAINmodel = linear_model.LinearRegression(fit_intercept=False)
X3 = X2.drop(columns = ['d5pGDPPC'])
X3["B0"] = 1
reg = MAINmodel.fit(X3, X2['d5pGDPPC'])
ission = pd.DataFrame(columns = X3.columns)
ission.loc[len(ission)] = reg.coef_
Arti = pd.DataFrame(columns = X.columns)
Arti['IVGinim5'] = range(1,100) ###This goes first!
...
Arti['IVGFDavg']  = Arti['FDavg'] * Arti['IVGinim5']
Arti['IVG2FDavg']  = Arti['FDavg'] * Arti['IVGini2m5']
#Arti['IVlnGG3m5'] = Arti['IVGini3m5'] * Arti['lnGDPPCm5']
"""
Arti['RelGDPPCm5'] = Arti['GDPPCm5'][0] / TGall[TGall.Year == Arti.Year[0]].GDPPCm5.max()
Arti['IVRelGGm5'] = Arti['RelGDPPCm5'] * Arti['IVGinim5']
...
"""
Arti['pred'] = ission.B0[0] + ission[str(Arti['Year'][0])][0] + ission[Arti['Country'][0]][0] #str(Arti['Year'][0]) gets fixed effect for that year
for column in Arti.drop(columns = ["Year", "Country", "d5pGDPPC", "pred"]): #, 'GDPPCm5'
    i = column
    Arti['pred'] = Arti['pred'] + (Arti[i]  * ission[i][0])
...
# %%
bar = sns.histplot((X[X.Year == 2018]).RelGDPPCm5, binwidth=.05, color = "orange")
bar.plot()
